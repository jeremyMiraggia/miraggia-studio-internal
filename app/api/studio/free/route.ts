...
function mapFramingToInstructions(cadrage: string): string {
  ...
  if (c.includes('haut') || c.includes('upper')) {
    return 'E-COMMERCE BUST SHOT (fashion catalogue / lookbook style), framing from the top of the head down to the WAIST / BELT LINE. ⚠ The ENTIRE top garment (t-shirt, blouse, sweater) is fully visible in frame, including its bottom hem and the belt area below it. Hands are visible — either at the sides, in pockets, or in front. NO legs, NO thighs in frame (cropped at waist). Comfortable breathing space around the model — this is NOT a tight close-up on the face. Leave 5-10% headroom above the head. Camera positioned at chest level, eye-line natural. VERTICAL EXTENT: from just above the head down to just below the belt. ' + BG_PRESERVE_NEUTRAL
  }
