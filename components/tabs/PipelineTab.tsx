...
export default function PipelineTab() {
    ...
    const processOne = async (item: TaskState): Promise<void> => {
      ...
      try {
        ...
        if (item.task.taskType === 'detail') {
          ...
          const resDetail = await fetch('/api/studio/pipeline', { method: 'POST', body: fdDetail })
        ...
        const res = await fetch('/api/studio/pipeline', { method: 'POST', body: fd })
  ...
  return (
    <div style={styles.wrap}>
      <h2 style={styles.title}>🔬 Pipeline modulaire — fond pixel-perfect garanti</h2>
      <div style={{ background: '#FFF3E0', border: '1px solid #F1C97A', borderRadius: 8, padding: 12, marginBottom: 12, fontSize: 13, color: '#7A4F00', lineHeight: 1.5 }}>
        <strong>⚡ Pipeline serveur :</strong> IDM-VTON (FAL.ai) habille le mannequin avec ton vêtement,
        puis BiRefNet le détoure, puis on le composite par-dessus <strong>ton fond exact</strong> (paste-back).
        Le fond Notion reste <strong>bit-pour-bit identique</strong> — aucune dérive possible.
        <br />
        ⏱ ~20s/visuel · 💰 ~$0.03/visuel · 🎯 Cohérence du fond garantie mécaniquement
      </div>
